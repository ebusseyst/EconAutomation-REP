...
class CaseProfile:
    ...
    def __post_init__(self):
        ...
        # Claimant name
        name_parts = [self.claimant_name_first, self.claimant_name_middle, self.claimant_name_last]
        self.claimant_name_full          = " ".join(p for p in name_parts if p)
        self.claimant_name_first_initial = self.claimant_name_first[0] if self.claimant_name_first else ""
        self.claimant_name_last_initial  = self.claimant_name_last[0]  if self.claimant_name_last  else ""
        ...
        sal, gen, subj, poss, obj = _gender_map.get(self.claimant_sex, ("", "", "", "", ""))
        ...
        self.claimant_salutation_with_name_full = f"{sal} {self.claimant_name_full}".strip()
        self.claimant_salutation_with_name_last = f"{sal} {self.claimant_name_last}".strip()
        
        # Claimant address
        addr_parts = [self.claimant_address_1, self.claimant_address_2]
        self.claimant_address_full = "\n".join(p for p in addr_parts if p)
        self.claimant_geozip       = self.claimant_address_2.split(" ")[-1] if self.claimant_address_2.strip() else ""
        ...
        self.case_manager_name_first_initial = self.case_manager_name_first[0] if self.case_manager_name_first else ""
        self.case_manager_name_last_initial  = self.case_manager_name_last[0]  if self.case_manager_name_last  else ""

        # Attorney name
        atty_parts = [self.attorney_name_first, self.attorney_name_middle, self.attorney_name_last]
        self.attorney_name_full          = " ".join(p for p in atty_parts if p)
        self.attorney_name_first_initial = self.attorney_name_first[0] if self.attorney_name_first else ""
        self.attorney_name_last_initial  = self.attorney_name_last[0]  if self.attorney_name_last  else ""
        ...
        self.attorney_salutation_with_name_full_with_title = f"Mr./Ms. {self.attorney_name_full}, Esq."
...
class ClaimantInfoExtractor:
    def __init__(self, OFF_pathstr: str):
        ...
        self.usable_pairs               = {}
        self.claimant_profile: CaseProfile | None = None
    ...
    def create_columns_dictionary(self) -> None:
        ...
        try:
            ...
            logger.exception("ClaimantInfoExtractor.create_columns_dictionary: FileNotFoundError")
            raise
    ...
    def clean_columns_dictionary(self) -> None:
        ...
        self.cleaned_columns_dictionary = {
            k: v for k, v in self.columns_dictionary.items()
            if any(cell != "" for cell in v)
        }

    def create_usable_pairs(self) -> None:
        """Zips adjacent columns into label→value pairs, handling address disambiguation."""
        all_lists     = list(self.cleaned_columns_dictionary.values())
        usable_pairs  = {}
        address_count = 0
        ...
        for list_1, list_2 in batched(all_lists, 2):
            for item_1, item_2 in zip(list_1, list_2):
                ...
                if item_1 == "Address:":
                    item_1 = "firm_address" if address_count == 0 else "claimant_address"
                    address_count += 1
    ...
    def _format_date(raw: str) -> tuple[str, str]:
        ...
        try:
            return raw, datetime.strptime(raw, "%m/%d/%Y").strftime("%B %d, %Y")
    ...
    def create_claimant_profile(self) -> None:
        ...
        """
        p = self.usable_pairs   # shorthand

        # Parse names
        claimant_first, claimant_middle, claimant_last = self._parse_name(p.get("Name:", ""))
        attorney_first, attorney_middle, attorney_last = self._parse_name(p.get("Referral Source:", ""))

        # Parse addresses
        firm_addr_1, firm_addr_2           = self._parse_address(p.get("firm_address", ""))
        claimant_addr_1, claimant_addr_2   = self._parse_address(p.get("claimant_address", ""))
        
        # Date parsing
        report_deadline_short, report_deadline_long = self._format_date(p.get("Report Deadline:", ""))
        addendum_report_deadline_short, addendum_report_deadline_long = self._format_date(p.get("Addendum Report Deadline:", ""))
        expert_designation_date_short, expert_designation_date_long = self._format_date(p.get("Expert Designation Date:", ""))
        mediation_date_short, mediation_date_long = self._format_date(p.get("Mediation Date:", ""))
        trial_date_short, trial_date_long = self._format_date(p.get("Trial Date:", ""))
        discovery_cutoff_short, discovery_cutoff_long = self._format_date(p.get("Discover Cutoff:", ""))
        date_opened_short, date_opened_long = self._format_date(p.get("Date Opened:", ""))

        self.claimant_profile = CaseProfile(
            # Claimant
            claimant_name_first   = claimant_first,
            claimant_name_middle  = claimant_middle,
            claimant_name_last    = claimant_last,
            claimant_sex          = self._expand_sex(p.get("Gender:", "")),
            claimant_address_1    = claimant_addr_1,
            claimant_address_2    = claimant_addr_2,
            claimant_phone        = p.get("Phone:", ""),
            claimant_DOB          = p.get("Date of Birth:", ""),
            claimant_age          = p.get("Age:", ""),
            claimant_DOI          = p.get("Injury Date:", ""),
            claimant_DOI_2        = p.get("Injury Date 2:", ""),
            claimant_injury_types = p.get("Injury Type:", ""),
            # Case
            case_number                       = p.get("Case #:", ""),
            case_type                         = _normalize_case_type(p.get("Case Type:", "")),
            case_manager_name_full            = p.get("Counselor:", ""),
            court_jurisdiction                = p.get("Court:", ""),
            billing_rate                      = p.get("Billing Rate:", ""),
            ...
            attorney_phone                    = p.get("Telephone #:", ""),
            attorney_fax                      = p.get("Fax #:", ""),
            attorney_email                    = p.get("Email #1:", ""),
            firm_name                         = p.get("Company Name:", ""),
